...
public class Easy extends Computer {
    ...
    if(columnCounter.size() == 1)
     ...
     else if(treatLevel2 >= treatLevel1 && treatLevel2 > treatLevel3 && treatLevel2 >= this.TREAT_LEVEL_REACT)
       place(columnCounterD);
      
      else if(treatLevel1 >= treatLevel3 && treatLevel1 >= TREAT_LEVEL_REACT)
        place(highestRowTreat);
     else if(treatLevel3 >= TREAT_LEVEL_REACT)
        place(listOfDiagonal);
      else
          playAttack(getBoard().getLastPlayedRow(), getBoard().getLastPlayedColumn());
     
      turnsPlayed++;
   }

    public boolean checkConsequences(){
         int rowNumber = getBoard().getLastPlayedRow();
   ...
   public void playAttack(int row, int column){
      
       int random = (int)( 1 + Math.random() * 3);
     if( random == 1){
       
       if(column > 0 && isRowDrop(column - 1, row )){
           placeToken(column - 1);
           column += 1;
           
       }
       else if(column < 6 && isRowDrop(column + 1, row )){
               placeToken(column + 1);
               column += 1;
       }
       else if(row > 0 && isRowDrop(column, row - 1)){
          placeToken(column);
          row -= 1;
               }
       else{
        ...
        if(row > 0 && isRowDrop(column, row - 1)){
          placeToken(column);
          row -= 1;
               }
      else if(column > 0 && isRowDrop(column - 1, row )){
           placeToken(column - 1);
       column -= 1;
      }
       else if(column < 6 && isRowDrop(column + 1, row )){
       ...
       if( random == 3){
           ...
           if(column < 6 && isRowDrop(column + 1, row )){
               placeToken(column + 1);
               column += 1;
           }
           else if(column > 0 && isRowDrop(column - 1, row )){
